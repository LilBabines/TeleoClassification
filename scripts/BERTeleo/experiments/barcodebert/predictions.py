...
def main(cfg: DictConfig):
    ...
    train_dataset, val_dataset, test_dataset, id2label, label2id = encode_singleTaxa_dataset(tokenizer,os.path.join(cfg.data.dataset_path,'fold_6'))
    ...
    model.load_state_dict(torch.load(r'C:\Users\Auguste Verdier\Desktop\TeleoClassification\scripts\BERTeleo\experiments\barcodebert\outputs\6_fold_single_barcodebert_teleo_clean\checkpoints\fold_6\checkpoint-84000\pytorch_model.bin'))
    args = TrainingArguments(os.path.join(log_dir,'checkpoints'),
                                **cfg.trainer.kwargs,
    ...
    print("Metrics on val set: ", result.metrics)    
    ...
    if cfg.task.save_preds :
        ...
        if cfg.task.task == "multiTaxa":
            ...
            import pickle
            pickle.dump({'preds':result.predictions,'labels':result.label_ids}, open(os.path.join(log_dir,'checkpoints','fold_6',"predictions.pkl"), 'wb'))
            dataframe.to_csv(os.path.join(log_dir,'checkpoints','fold_6',"predictions.csv"), index=False)
